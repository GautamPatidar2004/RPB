...
export const AIRecommendationCard: React.FC<AIRecommendationCardProps> = ({
  ...
  if (!activePlan && planningState !== 'CALCULATING') {
    return (
      <div className="bg-slate-950 border border-slate-800 rounded-lg p-5 flex flex-col items-center justify-center text-center h-full text-slate-400 select-none">
        <Sparkles size={32} className="text-purple-400/40 mb-2" />
        <div className="font-bold text-slate-300 text-sm">No Active AI Block Plan</div>
  ...
  return (
    <div className="bg-slate-950 border border-slate-800 rounded-lg flex flex-col h-full shadow-inner select-none overflow-hidden">
      ...
      <div className="p-3 border-b border-slate-800 bg-slate-900/60 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400">Multi-Strategy Pareto Winner · OR-Tools Verified</div>
          </div>
        </div>

        {/* Score Badge */}
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-slate-900 border border-purple-500/40">
      ...
      <div className="p-3 flex-1 overflow-y-auto flex flex-col gap-3 text-xs">
            ...
            <div className="mt-2 pt-2 border-t border-slate-800/80 space-y-1">
              ...
              <div className="max-h-28 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                {assignedTasks.map((t: any, i: number) => {
                  ...
                  return (
                    <div key={t.id || i} className="flex items-center justify-between bg-slate-950/80 p-1.5 rounded border border-slate-800 text-[10px]">
                      <div className="flex items-center space-x-1.5 overflow-hidden">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                        <span className="font-mono font-bold text-slate-200 shrink-0">{t.task_code || t.code || `TASK-${i+1}`}</span>
                        <span className="text-slate-400 truncate max-w-[130px]">{t.task_title || t.title}</span>
                      </div>
        ...
        <div className="p-2.5 rounded bg-slate-900/60 border border-slate-800 text-slate-300 leading-relaxed text-[11px]">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]">
          <div className="p-2 rounded bg-slate-900/60 border border-slate-800">
            ...
            <span className="text-[10px] text-slate-400 block mt-0.5">3 paths buffered</span>
          </div>
          ...
          <div className="p-2 rounded bg-slate-900/60 border border-slate-800">
            ...
            <span className="text-[10px] text-slate-400 block mt-0.5">Critical defects resolved</span>
          </div>
        </div>
